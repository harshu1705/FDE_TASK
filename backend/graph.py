...
def trace_order_flow(db, order_id: str) -> Dict[str, Any]:
    ...
    """)
        ...
        logger.warning(f"trace_order_flow DB error: {e}")
        # fallback minimal synthetic response so UI remains functional
        return {
            "order": {"order_id": order_id, "customer_id": "CUST123", "created_at": None},
            "deliveries": [],
            "invoices": [],
            "payments": []
        }
...
def get_entire_graph(db, limit=50) -> Dict[str, Any]:
    ...
    try:
        ...
        sample_nodes = [
            {"id": "order_1", "label": "Order 1", "group": "order"},
            {"id": "cust_1", "label": "Customer 1", "group": "customer"},
            {"id": "delivery_1", "label": "Delivery 1", "group": "delivery"},
            {"id": "invoice_1", "label": "Invoice 1", "group": "invoice"}
        ]
        sample_links = [
            {"source": "order_1", "target": "cust_1", "type": "placed_by"},
            {"source": "delivery_1", "target": "order_1", "type": "fulfills_order"},
            {"source": "invoice_1", "target": "order_1", "type": "bills_order"}
        ]
